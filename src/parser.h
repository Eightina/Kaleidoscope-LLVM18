            ...
            if (auto *defIR = defAST->codegen()) {
                ...
                if constexpr (CT == CompilerType::JIT) {
                    ...
                    auto tsm = llvm::orc::ThreadSafeModule(
                        std::move(_theModule), std::move(_theContext));
        ...
        if (auto fnAST = parseTopLevelExpr()) {
            if (auto *fnIR = fnAST->codegen()) {
                ...
                if constexpr (CT == CompilerType::JIT) {
                    ...
                    //  execution
                    auto rt =
                        _theJIT->getMainJITDylib().createResourceTracker();
                    auto tsm = llvm::orc::ThreadSafeModule(
                        std::move(_theModule), std::move(_theContext));
                    ...
                    auto exprSymbol =
                        _exitOnErr(_theJIT->lookup("__anon_expr"));
                ...
                } else if constexpr (CT == CompilerType::AOT) {
